...
<html lang="en" dir="ltr">
  ...
  <body>
    ...
    <form class="content" action="{{route('altausuario')}}" method="POST">
      ...
      Id Viaje: <input type="text" name="idviaje" onblur="fieldSize()" id="idviaje" pattern="[10 []]+" placeholder="Id Viaje">
      <p id="error"></p>
      Destino: <input type="text" name="destino" onblur="fieldSize1()" id="destino" pattern="[a-zA-Z]+" placeholder="Destino">
      <p id="error1"></p>
      Hora de Salida:<input type="time" name="hrs" onblur="fieldSize2()" id="hrs" pattern="[0-9 []]+" placeholder="Hora de Salida">
      <p id="error2"></p>
      Hora de Regreso: <input type="time" name="hrr" onblur="fieldSize3()" id="hrr" pattern="[0-9 []]+" placeholder="Hora de Regreso">
      <p id="error3"></p>
      Precio: <input type="text" name="precio" onblur="fieldSize4()" id="precio" pattern="[10 []]+" placeholder="Precio">
      <p id="error4"></p>
    ...
    const $input = document.querySelector("#idviaje");
    const patron = /[0-9]/;

    $input.addEventListener("keydown", event => {
      if (patron.test(event.key)){
        document.getElementById('idviaje').style.border = "1px solid #49AF13";
      }
      else {
        if (event.keyCode==8) {
    ...
    //------------------------------------
    function fieldSize() {
      var tam = document.getElementById('idviaje').value;
      if (tam>=1) {
        document.getElementById('error').innerHTML = "";
      }
      else {
        document.getElementById('error').innerHTML = "Error: Tamaño del texto.";
        document.getElementById('idviaje').style.border = "1px solid #EF0D0D";
      }
    }
    </script>
    ...
    <script type="text/javascript">
    const $input2 = document.querySelector("#hrs");
    const patron2 = /[a-zA-Z]+/;

    $input2.addEventListener("keydown", event => {
      if (patron2.test(event.key)){
        document.getElementById('hrs').style.border = "1px solid #49AF13";
      }
      else {
        if (event.keyCode==8) {
          //console.log("backspace");
    ...
    //------------------------------------
    function fieldSize2() {
      var apellido = document.getElementById('hrs').value;
      if (apellido.length>=5 && apellido.length<=30) {
        document.getElementById('error2').innerHTML = "";
      }
      else {
        document.getElementById('error2').innerHTML = "Error: Tamaño del texto.";
        document.getElementById('hrs').style.border = "1px solid #EF0D0D";
      }
    }
    </script>

    <script type="text/javascript">
    const $input3 = document.querySelector("#hrr");
    const patron3 = /[a-zA-Z]+/;

    $input3.addEventListener("keydown", event => {
      if (patron3.test(event.key)){
        document.getElementById('hrr').style.border = "1px solid #49AF13";
      }
      else {
        if (event.keyCode==8) {
          //console.log("backspace");
